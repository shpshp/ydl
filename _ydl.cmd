...
youtube-dl -f mp4 -a _ydl_720p.txt -o "%format%" --sub-lang %languages% --write-sub --write-auto-sub --embed-subs && rename _ydl_720p.txt _ydl_720p_%datetime%.txt && echo.>> _ydl_720p.txt
youtube-dl -f mp4 -a _ydl_mp3.txt -o "%format%" -x --audio-format "mp3" && rename _ydl_mp3.txt _ydl_mp3_%datetime%.txt && echo.>> _ydl_mp3.txt
